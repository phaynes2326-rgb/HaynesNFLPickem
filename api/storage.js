...
// Recomputes and stores every family member's score for a week, given the
// current results. Shared by setResult and the manual recompute action.
async function recomputeAllScores(week, weekResults) {
  const scores = {};
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body && body.action === 'setResult') {
        ...
        const scores = await recomputeAllScores(week, weekResults);
      ...
      // Commissioner-triggered fix: recalculates everyone's score for a week
      // from scratch, using current picks and results. Use this if a score
      // ever looks wrong or missed an update — doesn't touch anyone's picks.
      if (body && body.action === 'recomputeScores') {
        if (!commissionerPinValid(body.pin)) {
          return res.status(401).json({ error: 'wrong PIN' });
        }
        const { week } = body;
        ...
        const existingRaw = await redis.get(`results:week${week}`);
        const weekResults = existingRaw ? JSON.parse(existingRaw) : {};
        const scores = await recomputeAllScores(week, weekResults);
      ...
      if (isProtectedKey(key) && lock) {
        ...
        const week = weekFromPicksKey(key);
        if (week) {
          await redis.set(`locked:week${week}:${name}`, '1');
          // Compute this person's score immediately, using whatever results
          // already exist. Without this, someone who submits after the
          // commissioner has already entered results never gets scored,
          // since scoring otherwise only runs when a NEW result comes in.
          const existingResultsRaw = await redis.get(`results:week${week}`);
          const weekResults = existingResultsRaw ? JSON.parse(existingResultsRaw) : {};
